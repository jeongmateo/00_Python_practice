...
def add_file():
    files = filedialog.askopenfilenames(title="이미지 파일을 선택하세요", \
        ...
        initialdir=r"C:\Users\user\Desktop\Python") #최초 경로를 띄워줌
    #사용자가 선택한 파일목록
    for file in files:
...
def merge_image():
    ...
    try:
        ...
        if img_width > -1:
            image_size = [(int(img_width),int(img_width * x.size[1] / x.size[0])) for x in images]    #변경 된 width 값 사용
        else:
            image_size =[(x.size[0], x.size[1]) for x in images]    #원본 유지

        #계산식
        # 100 * 60 이미지가 있음. -> width 를 80으로 줄이면 height?
        # 원본 width : 원본 height = 변경 width : 원본 height
        # 100 : 60 = 80 : x  ---> x = (원본 height * 변경 width) / 원본 width
        

        # widths = [x.size[0] for x in images]
        # heights = [x.size[1] for x in images]
        #[(10,10),(20,20),(30,30)] -- (width,height)
        widths, heights = zip(*(image_size))

        #최대 넓이, 전체 높이 구해줌
        max_width, total_height = max(widths), sum(heights)
        print("max width: {}".format(max_width))
        print("sum height: {}".format(total_height))
        ...
        # 포맷 옵션 처리
        file_name = "Logan_Photo." + img_format
...
btn_add_file = Button(file_frame,padx=5,pady=5, width=15,text="파일추가",command=add_file)
btn_add_file.pack(side="left")

btn_del_file = Button(file_frame,padx=5,pady=5, width=15, text="선택삭제",command=del_file)
...
#2. 간격 옵션
# 간격 옵션 레이블
lbl_pad = Label(frame_option, text="간격",width=8)
lbl_pad.pack(side="left",padx=5, pady=5)

opt_pad = ["없음","좁게","보통","넓게"]
cmb_pad = ttk.Combobox(frame_option, state="readonly", values= opt_pad, width= 10)
cmb_pad.current(0)
cmb_pad.pack(side="left")
