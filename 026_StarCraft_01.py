...
class Marine(AttackUnit):
    ...
    # 스팀팩 :일정 시간 동안 공격 속도를 증가, 대신 체력 10 감소
...
class Tank(AttackUnit):
    ...
    def __init__(self):
        AttackUnit.__init__(self, "탱크", 150, 2, 20)
        self.seize_mode = False

    def set_seize_Mode(self):    # 더 높은 파워로 공격 가능, 이동불가
...
class Wraith(FlyableAttackUnit):
    def __init__(self):
        FlyableAttackUnit.__init__(self, "레이스", 80, 10, 10)
        self.clocked = True  # 클로킹 모드 (해제 상태)

    def clocking(self):
        if self.clocked == True:
            print("{0} : 클로킹 모드를 진행합니다.".format(self.name))
            self.clocked = False
        else:
